...
class CNN(nn.Module):
    def __init__(self, lr, epochs, batch_size, num_classes=10):
        ...
        self.optimizer = optim.Adam(self.parameters(), )    # for learning
        self.loss = nn.CrossEntropyLoss()                   # BinaryCrossEntropy - if 2 classes
        self.to(self.device)
        self.get_data()
        self.a = 0
    ...
    def get_data(self):
        ...
        self.train_data_loader = d.DataLoader(mnist_train_data,
                                              batch_size=self.batch_size,
                                              shuffle=True,
                                              num_workers=8)
        mnist_test_data = MNIST('mnist', train=False,
                                download=True, transform=ToTensor())
        self.test_data_loader = d.DataLoader(mnist_test_data,
                                             batch_size=self.batch_size,
                                             shuffle=True,
                                             num_workers=8)
